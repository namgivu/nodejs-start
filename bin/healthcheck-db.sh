...
AH=`cd "$SH/.." && pwd`  # AH aka APP_HOME
    ...
    source "$AH/docker/db/.config.sh"
        ...
        echo '--- ---'
        echo 'ensure postgres db working'
            docker exec $CONTAINER_NAME  psql postgresql://$DB_USER:$DB_PASS@:/$DB_NAME -c '\dt' -tA  #NOTE no DB_PORT here cause within the container, port is still 5432
echo "
--- ---
SUMMARY
